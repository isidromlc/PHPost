...
class tsActividad {
   ...
   */
   private function makeOracion($data = NULL){
      ...
      switch($ac_type){
         ...
         case 8:
            // AVATARES
            $img_uno = '<img width="16" height="16" src="'.$tsCore->settings['avatar'].'/'.$data['user_id'].'_120.jpg"/>';
            $img_dos = '<img width="16" height="16" src="'.$tsCore->settings['avatar'].'/'.$data['avatar'].'_120.jpg"/>';
            // ORACION
   ...
   */
   private function makeFecha($time = NULL){
      ...
      //
      return ($dias < 1) ? 'today' : ($dias < 2 ? 'yesterday' : ($dias < 7 ? 'week' : ($dias <= 30 ? 'month' : 'old')));
