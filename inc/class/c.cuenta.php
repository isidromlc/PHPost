...
class tsCuenta {
   ...
   */
   private function unData($data){
		$data['p_socials'] = unserialize($data['p_socials']);
     	$data['redes'] = ['facebook','twitter','instagram','twitch','youtube'];
	...
	*/
	function loadHeadInfo($user_id){
      ...
      $data['p_sitio'] = $tsCore->setSecure($data['p_sitio']);
		$data['p_socials'] = unserialize($data['p_socials']);
      $data['redes'] = ['facebook','twitter','instagram','twitch','youtube'];
		$data['p_configs'] = unserialize($data['p_configs']);
   ...
   */
   function iFollow($user_id){
      global $tsUser;
      // SEGUIR
      $data = db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$user_id.'\' AND f_user = \''.(int)$tsUser->uid.'\' AND f_type = \'1\' LIMIT 1'));
    ...
    */
    function yFollow($user_id){
      global $tsUser;
      // YO LE SIGO?
      $data = db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT follow_id FROM u_follows WHERE f_id = \''.(int)$tsUser->uid.'\' AND f_user = \''.(int)$user_id.'\' AND f_type = \'1\' LIMIT 1'));
      return ($data > 0) ? true : false;
    }
   /*
      loadPosts($user_id)
   */
   function loadPosts($user_id){
      global $tsUser;
      //
      $data['posts'] = result_array(db_exec(array(__FILE__, __LINE__), 'query', 'SELECT p.post_id, p.post_title, p.post_puntos, c.c_seo, c.c_img FROM p_posts AS p LEFT JOIN p_categorias AS c ON c.cid = p.post_category WHERE p.post_status = \'0\' AND p.post_user = \''.(int)$user_id.'\' ORDER BY p.post_date DESC LIMIT 18'));
   ...
   */
   function loadMedallas($user_id){
      //
		$data['medallas'] = result_array(db_exec(array(__FILE__, __LINE__), 'query', 'SELECT m.*, a.* FROM w_medallas AS m LEFT JOIN w_medallas_assign AS a ON a.medal_id = m.medal_id WHERE a.medal_for = \''.(int)$user_id.'\' AND m.m_type = \'1\' ORDER BY a.medal_date DESC'));
	...
	function savePerfil(){
		...
		switch($save){
            ...
            if(!empty($sitio)) $sitio = substr($sitio, 0, 4) == 'http' ? $sitio : 'http://'.$sitio;
				...
				$twitch = $tsCore->setSecure($tsCore->parseBadWords($_POST['twitch']), true);
         ...
         case 'seguridad':
            ...
            elseif($new_passwd != $confirm_passwd) return array('error' => 'Tu nueva contrase&ntilde;a debe ser igual a la confirmaci&oacute;n de la misma.');
               ...
               $verify = $tsCore->passwordVerify($_POST['passwd'], $tsUser->nick, $tsUser->info['user_password']);
               if(!$verify):
                  return array('error' => 'Tu contrase&ntilde;a actual no es correcta.');
               else:
				      $new_key = $tsCore->passwordHash($new_passwd, strtolower($tsUser->nick), 8);
        ...
        case 'username':
            ...
            $datos_nuevos = array(
               ...
               "contrasena" => $tsCore->setSecure($_POST['password']),
               "correo" => $tsCore->setSecure($_POST['pemail'])
            );
            if(db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT id FROM w_blacklist WHERE type = \'4\' && LOWER(value) = \''.$datos_nuevos['nickNuevo'].'\' LIMIT 1'))) return array('error' => 'Nick no permitido');
            if(db_exec('num_rows', db_exec(array(__FILE__, __LINE__), 'query', 'SELECT user_id FROM u_miembros WHERE LOWER(user_name) = \''.$datos_nuevos['nickNuevo'].'\' LIMIT 1'))) return array('error' => 'Nombre en uso');
				...
				$verify = $tsCore->passwordVerify($datos_nuevos['contrasena'], $tsUser->nick, $tsUser->info['user_password']);
				if(!$verify) return array('error' => 'Tu contrase&ntilde;a actual no es correcta.');
            else {		
					$email_ok = $this->isEmail($datos_nuevos['correo']);
				if(!$email_ok) return array('field' => 'email', 'error' => 'El formato de email ingresado no es v&aacute;lido.');
					$email = empty($datos_nuevos['correo']) ? $tsUser->info['user_email'] : $datos_nuevos['correo'];
					if(strlen($datos_nuevos['nickNuevo']) < 4 || strlen($datos_nuevos['nickNuevo']) > 16) return array('error' => 'El nick debe tener entre 4 y 16 car&aacute;cteres');
					if(!preg_match('/^([A-Za-z0-9]+)$/', $datos_nuevos['nickNuevo'])) return array('error' => 'El nick debe ser alfanum&eacute;rico');
					$key = $tsCore->passwordHash($datos_nuevos['contrasena'], strtolower($datos_nuevos['nickNuevo']), 8);
					$_SERVER['REMOTE_ADDR'] = $_SERVER['X_FORWARDED_FOR'] ? $_SERVER['X_FORWARDED_FOR'] : $_SERVER['REMOTE_ADDR'];
               if(!filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) return array('error' => 'Su IP no se pudo validar');
					if(db_exec(array(__FILE__, __LINE__), 'query', 'INSERT INTO `u_nicks` (`user_id`, `user_email`, `name_1`, `name_2`, `hash`, `time`, `ip`) VALUES (\''.$tsUser->uid.'\', \''.$tsCore->setSecure($email).'\', \''.$tsUser->nick.'\', \''.$datos_nuevos['nickNuevo'].'\', \''.$key.'\', \''.time().'\', \''.$tsCore->setSecure($_SERVER['REMOTE_ADDR']).'\')')) return array('error' => 'Proceso iniciado, recibir&aacute; la respuesta en el correo indicado cuando valoremos el cambio.');
				}
         break;
		}
		// ACTUALIZAR
