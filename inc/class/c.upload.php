...
class tsUpload {
    ...
    var $max_size = 1048576 * 2;    // 2MB 
    var $allow_types = array('png','gif','jpeg'); // ARCHIVOS PERMITIDOS
    var $found = 0; // VARIABLE BANDERA 
    var $file_url = ''; // URL
    var $file_size = []; // TAMA�O DEL ARCHIVO REMOTO
    var $image_size = array('w' => 600, 'h' => 600);
   ...
   */
   function uploadFile($file, $type = 'file'){
      ...
      if(!empty($error)) {
         ...
         $newName = 'avatar_'.$key.'.'.$ext;
         // IMAGEN
         if($this->type == 1) return array(1, $this->sendFile($file,$newName), $type[1]);
         // CROP
         else return array('msg' => $this->createImage($file,$newName), 'error' => '', 'key' => $key, 'ext' => $ext);
          //
      }
   }
   /*
   ...
   */
   function validFile($file, $type = 'file'){
      ...
      if($type == 'file'){
         ...
         elseif($this->file_size[0] < $min || $this->file_size[1] < $min) return 'Tu foto debe tener un tama&ntilde;o superior a '.$min.'x'.$min.' pixeles.';
         elseif($this->file_size[0] > $max || $this->file_size[1] > $max) return 'Tu foto debe tener un tama&ntilde;o menor a '.$max.'x'.$max.' pixeles.';
      }
      // TODO BIEN
      return false;
   }
   ...
   */
   public function cropAvatar($key){
      $source = TS_FILES.'uploads/avatar_'.$_POST['key'].'.'.$_POST['ext'];
