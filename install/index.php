...
switch ($step) {
   ...
   case 2:
      ...
      if ($_POST['save']) {
         ...
         if (empty($db_link)) {
            ...
            if (mysqli_fetch_row(mysqli_query($db_link, 'SHOW TABLES LIKE \'w_configuracion\'')) == true) header('Location: ./../upgrade/index.php');
            else {
               // GUARDAR LOS DATOS DE CONEXION
               $config = file_get_contents('../config.inc.php');
               $config = str_replace(['dbhost', 'dbuser', 'dbpass', 'dbname'], [$dbhost, $dbuser, $dbpass, $dbname], $config);
               file_put_contents('../config.inc.php', $config);
               // INSERTAR DB
               include 'database.php';
               $bderror = '';
               foreach ($phpos_sql as $key => $sql) {
                  if (mysqli_query($db_link, $sql)) $exe[$key] = 1;
                  else {
                     $exe[$key] = 0;
               ...
               else $message = 'Lo sentimos, pero ocurrió un problema. Inténtalo nuevamente; borra las tablas que se hayan guardado en tu base de datos: ' . $bderror;
            }
         }
      }
   break;
   ...
   case 3:
      ...
      if ($_POST['save']) {
         ...
         if (empty($wname) || empty($wlema) || empty($wurl) || empty($wmail) || empty($pkey) || empty($skey)) $message = 'Todos los campos son requeridos';
         else {
            ...
            if ($db['hostname'] != 'dbhost') {
               if (!mysqli_num_rows(mysqli_query($db_link, "SELECT user_id FROM u_miembros WHERE user_id = 1 || user_rango = 1"))) {
                  ...
                  } else $message = mysqli_error($db_link);
   ...
   case 4:
      ...
      if ($_POST['save']) {
         ...
         if (empty($uname) || empty($upass) || empty($ucpass) || empty($umail)) $message = 'Todos los campos son requeridos';
            ...
            if (!ctype_alnum($uname)) $message = 'Introduzca un nombre de usuario alfanum&eacute;rico';
            else {
               if (!filter_var($umail, FILTER_VALIDATE_EMAIL)) $message = 'Introduzca un email correcto.';
               else {
                  // PASSWORD
   ...
   case 5:
      ...
      if ($_POST['save']) header("Location: {$data['url']}");
      else {
         // CONSULTA
...
<body>
   <div id="container">
      <div id="header">
         <h1 class="s32 left"><a href="http://www.phpost.net/foro/" target="_blank"><img src="./logo.png" /></a></h1>
      ...
      <div id="content">
         <div class="col_left">
            ...
            <ul class="menu">
               ...
               <li id="mstep_1"<?php if ($step == 5) echo ' class="ok"'; ?>>Bienvenid0</li>
            </ul>
         </div>
         <div class="col_right">
            <div id="step_<?php echo $step; ?>" class="step">
               ...
               <?php } elseif ($step == 1) { ?>
                  <form action="index.php<?php if ($next == true) echo '?step=2'; ?>" method="post" id="form">
                     <fieldset>
                        ...
                        <p><input type="submit" class="gbqfb" value="<?php echo ($next == true) ? 'Continuar &raquo;' : 'Volver a verificar'; ?>"/></p>
                     </fieldset>
                  </form>
               <?php } elseif ($step == 2) { ?>
                  <form action="index.php?step=<?php echo ($next == true) ? '3' : '2'; ?>" method="post" id="form">
                     <fieldset>
                        ...
                        <dl>
                           ...
                           <dd><input type="text" autocomplete="off" id="f1" placeholder="localhost" name="dbhost" value="<?php echo $dbhost; ?>" required/></span></dd>
                        </dl>
                        <dl>
                           <dt><label for="f2">Usuario:</label><br /><span>El usuario de tu base de datos.</span></dt>
                           <dd><input type="text" autocomplete="off" id="f2" placeholder="root" name="dbuser" value="<?php echo $dbuser; ?>" required/></span></dd>
                        </dl>
                        <dl>
                        ...
                        <dl>
                           ...
                           <dd><input type="text" autocomplete="off" id="f4" placeholder="my_database" name="dbname" value="<?php echo $dbname; ?>" required/></span></dd>
                        </dl>
                        <p><input type="submit" class="gbqfb" name="save" value="Continuar &raquo;"/></p>
                     </fieldset>
                  </form>
               <?php } elseif ($step == 3) {  ?>
                  <form action="index.php?step=<?php echo ($next == true) ? '4' : '3'; ?>" method="post" id="form">
                     <fieldset>
               ...
               <?php } elseif ($step == 4) { ?>
                  <form action="index.php?step=<?php echo ($next == true) ? '5' : '4'; ?>" method="post" id="form">
                     <fieldset>
      ...
      <div id="footer">
         <p>Powered by <a href="http://www.phpost.net/foro/" target="_blank">PHPost</a></p>
      </div>
   </div>
</body>
</html>
