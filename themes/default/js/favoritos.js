...
var favoritos = {
	...
	printResult: function(){
		...
		$.each(this.r, function(i, favorito){
			var h = favoritos.template_favorito.replace(/__fav_id__/g, favorito['fav_id']).replace(/__imagen__/g, favorito['imagen']).replace(/__categoria_name__/g, favorito['categoria_name']).replace(/__titulo__/g, favorito['titulo']).replace(/__url__/g, favorito['url']).replace(/__fecha_creado_formato__/g, favorito['fecha_creado_formato']).replace(/__fecha_creado_palabras__/g, favorito['fecha_creado_palabras']).replace(/__fecha_guardado_formato__/g, favorito['fecha_guardado_formato']).replace(/__fecha_guardado_palabras__/g, favorito['fecha_guardado_palabras']).replace(/__puntos__/g, my_number_format(favorito['puntos'])).replace(/__comentarios__/g, my_number_format(favorito['comentarios']));
			el.append(h);
		});
	},
	...
	query: function(force_no_parcial){
		...
		if(!parcial || this.orden_anterior != this.orden || this.eliminados_force_order)
			this.r = this.r.orderBy((this.orden=='titulo' ? '' : '-') + this.orden); //Si ordena por titulo ASC, todos los demas DESC
			...
			success: function(h){
				switch(h.charAt(0)){
					...
					case '1': //OK
						...
						$(obj).children().attr({'src': global_data.img + 'images/borrar.png',
																		'title': 'Borrar',
																		'alt': 'Borrar'
																	});
